...
const JobPrediction = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const predictionResponse = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/predictions/predict/`, submissionData, {
        headers: {
      ...
      if (predictionResponse.data) {
        ...
        try {
          ...
          const historyResponse = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/predictions/history/save/`, historyData, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      try {
        const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/predictions/feedback/`, feedbackData, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
      ...
      if (predictionId) {
        try {
          ...
          const predictionResponse = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/predictions/history/${predictionId}/`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  const handleSavePrediction = async () => {
    ...
    try {
      ...
      try {
        await axios.patch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/predictions/history/${predictionId}/`, {
          is_saved: true
        }, {
          headers: {
            'Authorization': `Bearer ${token}`,
